...
import { formatSetEffectKeys } from "helpers/artifacts";
...
function ArtifactListRow({
    ...
    const { name, displayName, setEffect } = artifact;

    const href = `/artifacts/${name.split(" ").join("_").toLowerCase()}`;
        ...
        >
            <Grid size={{ xs: 12, md: 3 }}>
                ...
                >
                    <RouterLink to={href}>
                        <Image
                            src={`artifacts/sets/${name.split(" ").join("_")}/${
...
export default ArtifactListRow;
